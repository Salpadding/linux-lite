...
class reg_parser:
    ...
    def parse_desc(self, **kwargs):
        ...
        gdt_entries = struct.unpack('<' + 'Q' * (len(mem) // 8), mem)
        return [self.gdt_entry(x) for x in gdt_entries]
    ...
    def gdt_entry(self, num):
        ...
        if not (access & (1 << 4)):
            types = ['', '16TSS(Avail)', 'LDT', '16TSS(Busy)', '', '', '', '', '', '32TSS(Avail)', '', '32TSS(Busy)']
